...
\title{Layer based routing to obtain quality optimization of scalable video streaming in the Future Internet}
...
Quality of Service (\textit{QoS}) support for Scalable Video Streaming (\textit{SVC}) in an OpenFlow network. 
...
Second, we provide failure recovery so the OpenFlow network can converge back to an optimal 
...
require no packet loss and minimal network delay due to their high Quality of Service (QoS) dependency. 
When these QoS flows are sent over the same path as other best effort traffic they 
are subject to network congestion and delay which renders an unsatisfactory outcome. 
Several efforts have been made to support QoS in a TCP/IP-based network, such as DiffServ and IntServ, but these 
solutions only work well in managed networks.
Scalable Video Coding (SVC) encodes the stream in a base layer along with several enhancement layers. 
In order to obtain correct output it is important that the base layer has no 
packet loss and minimal delay, which is not possible without support for QoS. 
To solve this issue we need the routing algorithms to detect QoS flows and route these flows differently than 
normal best effort traffic.
To achieve this goal in video streaming, we route the base layer of SVC encoded video at a high QoS 
while the enhancement layers are routed either at a lower QoS or as a best effort flow. 
We also provide failure recovery so the network 
can converge back to an optimal state when links go down.\\

We use OpenFlow in order to provide QoS support for scalable video streaming. 
...
The implementation of the controller we will use is Floodlight, because it is open source and widely used.
...
In section III we take a closer look at the components of our framework 
and the principals it is based on. In section IV we present the different algorithms 
used to determine the behavior of each flow in the network. Section V shows the test 
scenario we used to evaluate our framework while Section VI presents results.
In Section VII we come to a conclusion.

\section{Related work}
In our research we identified a need for separate forwarding actions per flow.
The same principal has been discussed in research papers from Princeton University. 
In their research the aim was to apply load-balancing during service discovery of web services 
...
for service discovery, which leads to network congestion. By using OpenFlow, the researchers at 
Princeton University managed to set separate forwarding actions for client requests which match 
...
When both best effort and QoS flows follow the same paths, chances for congestion and packet loss 
to occur increase significantly. OpenFlow allows us to set different paths for both flows, which 
...
research described in [4] and [5]. By routing the base layer as a lossless-QoS flow we can 
guarantee a minimal quality to the user. Ideally, the enhancement layers will also be delivered 
without packet loss or delay but when congestion occurs we demote the enhancement layers to best 
effort traffic and prioritize the base layer. Research discussed in [4] also attempts to route 
the enhancement layers as a lossy-QoS flow. Our research goes further and also enables fast failure 
recovery when a link fails.\\

\section{Architecture}
Each OpenFlow network contains at least one controller which is responsible for managing all the switches. 
A default OpenFlow switch forwards packets 
to the controller if the packet does not match any forwarding entry. 
We route SVC encoded video streams as a QoS flow, where we consider the base layer as a 
high priority QoS flow and the enhancement layer as either a lower QoS flow or as best effort traffic. 
These principals are shown in Figure \ref{fig_sim}. A filter on the controller separates QoS flows from 
other traffic, after which the controller can choose a path for the different layers. 
...
We defined a cost function which can be used to calculate the cost of a path.
The cost function is based on the following observations. (Insert ref here)
\begin{itemize}
...
These observations show that packet loss and path length are part of our cost function: 
the cost function must find a balance between maintaining low packet loss of the best effort 
traffic and the delay of QoS flows, caused by using longer paths.\\

Furthermore, the controller 
must be able to obtain these metrics at any time from any switch in the network before calculating 
new flow tables. We implemented a monitor module on the controller to gather all network 
statistics. Besides polling for statistics, a switch must also forward events to the controller when 
they occur, such as an link going down or a flow experiencing packet loss. These events provide 
additional monitoring data for the routing algorithms on the controller.\\

\section{QoS Routing}
Must ensure zero packet loss base layer, optimize other traffic ... cost function?
