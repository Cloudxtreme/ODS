...
require no packet loss and minimal network delay. 
...
can converge back to an optimal state when links fail.\\

We use OpenFlow \cite{mckeown2008openflow} in order to provide QoS support for scalable video streaming. 
...
When both best effort and QoS flows follow the same path, chances of congestion and packet loss 
occurring increase significantly. OpenFlow allows us to set different paths for both flows, which 
...
By routing the base layer as a lossless-QoS flow we can 
...
effort traffic and prioritize the base layer. Research discussed in \cite{egilmez2011scalable} 
also attempts to route the enhancement layers as a lossy-QoS flow. 
Our research goes further and also enables fast failure 
...
The cost function is based on the following observations.
\begin{itemize}
