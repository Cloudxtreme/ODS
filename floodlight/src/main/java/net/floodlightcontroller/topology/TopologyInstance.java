...
public class TopologyInstance {
	...
	protected static Logger log = LoggerFactory
			.getLogger(TopologyInstance.class);

	protected Map<Long, Set<Short>> switchPorts; // Set of ports for each switch
	/**
	 * Set of switch ports that are marked as blocked. A set of blocked switch
	 * ports may be provided at the time of instantiation. In addition, we may
	 * add additional ports to this set.
	 */
	protected Set<NodePortTuple> blockedPorts;
	protected Map<NodePortTuple, Set<Link>> switchPortLinks; // Set of links
																// organized by
																// node port
																// tuple
	...
	public void printTopology() {
		if (log.isTraceEnabled()) {
			...
			log.trace("clusterBroadcastNodePorts: {}",
	...
	protected void addLinksToOpenflowDomains() {
		for (long s : switches) {
			...
			for (short p : switchPorts.get(s)) {
				...
				if (switchPortLinks.get(np) == null)
					continue;
				if (isBroadcastDomainPort(np))
					continue;
	 ...
	 *         a strongly connected component. The network may contain
	 *         unidirectional links. The function calls dfsTraverse for
	 *         performing depth first search and cluster formation.
	 * 
	 ...
	 * @author Srinivasan Ramasubramanian
	 * 
	 *         This algorithm computes the depth first search (DFS) traversal of
	 *         the switches in the network, computes the lowpoint, and creates
	 *         clusters (of strongly connected components).
	 * 
	 *         The computation of strongly connected components is based on
	 *         Tarjan's algorithm. For more details, please see the Wikipedia
	 *         link below.
	 ...
	 * 
	 *         A return value of -1 indicates that dfsTraverse failed somewhere
	 *         in the middle of computation. This could happen when a switch is
	 *         removed during the cluster computation procedure.
	 * 
	 * @param parentIndex
	 *            : DFS index of the parent node
	 * @param currIndex
	 *            : DFS index to be assigned to a newly visited node
	 * @param currSw
	 *            : ID of the current switch
	 * @param dfsList
	 *            : HashMap of DFS data structure for each switch
	 * @param currSet
	 *            : Set of nodes in the current cluster in formation
	...
	private long dfsTraverse(long parentIndex, long currIndex, long currSw,
		...
		if (switchPorts.get(currSw) != null) {
			for (Short p : switchPorts.get(currSw)) {
				Set<Link> lset = switchPortLinks.get(new NodePortTuple(currSw,
						p));
				if (lset == null)
					continue;
				for (Link l : lset) {
					...
					if (dstDFS.getDfsIndex() < currDFS.getDfsIndex()) {
						...
						// make a DFS visit
						currIndex = dfsTraverse(currDFS.getDfsIndex(),
								currIndex, dstSw, dfsList, myCurrSet);
	...
	/**
	 * Go through every link and identify it is a blocked link or not. If
	 * blocked, remove it from the switchport links and put them in the blocked
	 * link category.
	 * 
	 * Note that we do not update the tunnel ports and broadcast domain port
	 * structures. We need those to still answer the question if the ports are
	 * tunnel or broadcast domain ports.
	 * 
	 * If we add additional ports to blocked ports later on, we may simply call
	 * this method again to remove the links on the newly blocked ports.
	 */
	...
	protected void calculateBroadcastNodePortsInClusters() {
		...
		for (Cluster c : clusters) {
			...
			for (long nodeId : links.keySet()) {
				...
				if (l == null)
					continue;
				NodePortTuple npt1 = new NodePortTuple(l.getSrc(),
						l.getSrcPort());
				NodePortTuple npt2 = new NodePortTuple(l.getDst(),
						l.getDstPort());
				nptSet.add(npt1);
				nptSet.add(npt2);
			}
	...
	public boolean isConsistent(long oldSw, short oldPort, long newSw,
			short newPort) {
		if (isInternalToOpenflowDomain(newSw, newPort))
			return true;
